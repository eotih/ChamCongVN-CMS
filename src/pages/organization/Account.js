...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(
      array,
      (_user) => _user.Employee.FullName.toLowerCase().indexOf(query.toLowerCase()) !== -1
    );
  }
...
export default function Account() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = filteredUsers.map((n) => n.Account.AccountID);
  ...
  const handleClick = (event, name) => {
    const selectedIndex = selected.indexOf(name.Account.AccountID);
    let newSelected = [];
    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selected, name.Account.AccountID);
  ...
  const handleDelete = (data) => {
    if (confirm(`Are you sure you want to delete ${selected.length} account?`)) {
      const list = selected.map((item) => {
        if (item.headline === data.headline) {
          axios.delete(`Organization/Account/${item}`).then((res) => {
            if (res.data.Status === 200) {
              setOpen(false);
              handleOpenToast({
                isOpen: true,
